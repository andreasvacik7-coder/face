...
Mit Multi-Face-Auswahl, Gesichts-Markierungen, Smart Caching und Background Processing.
"""
import os
import time
import hashlib
...
class SmartCache:
    """
    Intelligenter Cache für Face-Recognition Operationen.
    Cached Face-Encodings, Suchergebnisse und Background-Tasks.
    """
    ...
    def __init__(self, cache_dir: str = ".cache"):
        ...
        self.cache_ttl = timedelta(hours=24)  # 24h Cache-Lebensdauer
        ...
        # Session State für Streamlit Integration
    ...
    def _get_hash(self, data: Union[bytes, str, np.ndarray]) -> str:
        """Erstelle eindeutigen Hash für Cache-Key."""
    ...
    @st.cache_data(ttl=3600)  # Streamlit Cache für 1h
    def get_face_encodings(_self, image_hash: str, image_data: bytes) -> Optional[List[Dict]]:
        """
        Cache Face-Encodings mit Streamlit Cache + File Cache.
        """
        cache_key = f"face_enc_{image_hash}"
        cache_path = _self._get_cache_path(cache_key)
    ...
    def store_face_encodings(self, image_hash: str, encodings: List[Dict]):
        """Speichere Face-Encodings in Cache."""
        cache_key = f"face_enc_{image_hash}"
        cache_path = self._get_cache_path(cache_key)
        
        # Memory Cache (mit Größenbegrenzung)
        ...
        try:
            ...
            st.warning(f"Cache-Speichern fehlgeschlagen: {e}")
    
    @st.cache_data(ttl=1800)  # 30min Cache für Suchergebnisse
    def get_search_results(_self, query_hash: str, threshold: float) -> Optional[List[Dict]]:
        """Cache Suchergebnisse."""
        cache_key = f"search_{query_hash}_{threshold}"
        cache_path = _self._get_cache_path(cache_key)
    ...
    def store_search_results(self, query_hash: str, threshold: float, results: List[Dict]):
        ...
        try:
            ...
            st.warning(f"Search-Cache fehlgeschlagen: {e}")
    
    def start_background_task(self, task_id: str, func, *args, **kwargs) -> str:
        ...
        st.session_state.cache_stats['background_tasks'] += 1
        return task_id
    
    def get_background_result(self, task_id: str) -> Optional[Dict]:
        """Hole Ergebnis eines Background-Tasks."""
        ...
        if future.done():
            ...
            return {'status': 'running', 'started': task['started']}
    ...
    def get_cache_stats(self) -> Dict:
        ...
        stats['background_active'] = len([t for t in self.background_tasks.values() 
                                         if t['status'] == 'running'])
...
class SmartFaceSearch:
    """
    Intelligente Gesichtssuche mit Multi-Level-Verifikation, Smart Caching und Background Processing.
    ...
    def extract_all_faces(self, image_data, use_cnn=True) -> List[Dict]:
        ...
        Mit Smart Caching für bessere Performance.
        """
        ...
        try:
            ...
            if hasattr(image_data, 'read'):
                ...
                # Konvertiere PIL Image zu bytes für Hash
            ...
            # Cache Check
            image_hash = self.cache._get_hash(image_bytes)
            
            if self.stats['cache_enabled']:
                cached_faces = self.cache.get_face_encodings(image_hash, image_bytes)
                if cached_faces is not None:
                    st.success("⚡ Cache-Hit: Gesichter aus Cache geladen!")
                    return cached_faces
            
            st.write(f"🔍 **Debug:** Analysiere neues Bild (Hash: {image_hash[:8]}...)")
            ...
            # Background Processing für große Bilder
            if self.stats['background_enabled'] and (height * width) > 1000000:  # > 1MP
                task_id = f"face_detection_{image_hash}"
                existing_task = self.cache.get_background_result(task_id)
                
                if existing_task and existing_task['status'] == 'running':
                    st.info("🔄 Gesichtserkennung läuft im Hintergrund... (große Bildauflösung)")
                    return []
                elif existing_task and existing_task['status'] == 'completed':
                    st.success("⚡ Background-Task abgeschlossen!")
                    faces = existing_task['result']
                    if self.stats['cache_enabled']:
                        self.cache.store_face_encodings(image_hash, faces)
                    return faces
            ...
            if self.stats['cache_enabled']:
                self.cache.store_face_encodings(image_hash, faces)
                st.write(f"💾 **Debug:** Gesichter in Cache gespeichert")
    ...
    def smart_search(self, query_encoding: np.ndarray, progress_callback=None) -> List[Dict]:
        ...
        Mit Smart Caching, Background Processing und Performance Monitoring.
        """
        start_time = time.time()
        self.stats['total_searches'] += 1
        
        # Cache Check für Suchergebnisse
        query_hash = self.cache._get_hash(query_encoding)
        threshold = self.quality_thresholds['acceptable']
        
        if self.stats['cache_enabled']:
            cached_results = self.cache.get_search_results(query_hash, threshold)
            if cached_results is not None:
                if progress_callback:
                    progress_callback(1.0, "⚡ Cache-Hit: Suchergebnisse aus Cache geladen!")
        ...
        if vector_db_ready:
            ...
            # Optional: Verifiziere unsichere Ergebnisse im Hintergrund
            uncertain_results = [r for r in results if r['dist'] > self.quality_thresholds['very_good']]
            if (len(uncertain_results) > 0 and len(uncertain_results) <= 20 and 
                self.stats['background_enabled']):
                
                # Starte Background-Verifikation
                task_id = f"verify_{query_hash}_{int(time.time())}"
                self.cache.start_background_task(
                    task_id, 
                    self.verify_results, 
                    results, 
                    query_encoding, 
                ...
                if progress_callback:
                    progress_callback(0.9, f"🔄 Verifikation läuft im Hintergrund ({len(uncertain_results)} Ergebnisse)")
        ...
        # Speichere in Cache
        if self.stats['cache_enabled']:
            self.cache.store_search_results(query_hash, threshold, final_results)
    ...
    def _update_performance_stats(self, search_time: float):
        ...
        # Rolling Average für Suchzeit
        current_avg = self.stats['avg_search_time']
        total_searches = self.stats['total_searches']
        
        # Exponential Moving Average
        alpha = 2.0 / (min(total_searches, 20) + 1)  # Berücksichtige letzte 20 Suchen
        self.stats['avg_search_time'] = (1 - alpha) * current_avg + alpha * search_time
...
class FaceSearchUI:
    ...
    Mit Performance Dashboard und Smart Caching.
    """
    ...
    def render_performance_dashboard(self):
        ...
        with col3:
            st.metric("🕒 Ø Suchzeit", f"{search_stats['avg_search_time']:.2f}s")
        with col4:
            st.metric("🔍 Gesamt-Suchen", search_stats['total_searches'])
        
        # Erweiterte Performance-Info
        with st.expander("🔧 Erweiterte Performance-Einstellungen"):
            ...
            with col1:
                ...
                st.write(f"• Background Tasks: {cache_stats['background_tasks']}")
                st.write(f"• Active Tasks: {cache_stats['background_active']}")
            
            with col2:
                st.write("**Performance-Optimierungen:**")
                
                # Cache Ein/Aus
                cache_enabled = st.checkbox("🧠 Smart Caching aktiviert", 
                                          value=search_stats['cache_enabled'])
                self.search_engine.stats['cache_enabled'] = cache_enabled
                
                # Background Processing Ein/Aus
                bg_enabled = st.checkbox("🔄 Background Processing aktiviert", 
                                       value=search_stats['background_enabled'])
                self.search_engine.stats['background_enabled'] = bg_enabled
                
                # Cache aufräumen
                if st.button("🧹 Cache aufräumen"):
                    self.cache.cleanup_old_cache()
                    st.success("✅ Cache aufgeräumt!")
    
    def render_advanced_upload_section(self) -> Optional[Dict]:
        """Erweiterte Upload-Sektion mit Batch-Upload."""
        st.subheader("🎯 Gesicht(er) zum Suchen hochladen")
        
        # Upload-Modi
        upload_mode = st.radio(
            "Upload-Modus:",
            ["📸 Einzelbild", "📚 Mehrere Bilder (Batch)"],
            horizontal=True
        )
        
        if upload_mode == "📸 Einzelbild":
            return self._render_single_upload()
        else:
            return self._render_batch_upload()
    
    def _render_single_upload(self) -> Optional[Dict]:
        """Standard Einzelbild-Upload."""
    ...
    def _render_batch_upload(self) -> Optional[Dict]:
        """Batch-Upload für mehrere Bilder."""
        uploaded_files = st.file_uploader(
            "Wähle mehrere Bilder für Batch-Verarbeitung",
            type=['png', 'jpg', 'jpeg', 'heic', 'heif', 'bmp', 'webp'],
            accept_multiple_files=True,
            help="Lade mehrere Bilder hoch - alle Gesichter werden erkannt"
        )
        
        if uploaded_files:
            st.info(f"📚 {len(uploaded_files)} Bilder hochgeladen")
            
            # Background Processing für Batch
            batch_hash = self.cache._get_hash(str([f.name for f in uploaded_files]))
            task_id = f"batch_process_{batch_hash}"
            
            # Prüfe ob Background-Task läuft
            task_result = self.cache.get_background_result(task_id)
            
            if task_result is None:
                # Starte Background-Task
                if st.button("🚀 Batch-Verarbeitung starten"):
                    st.info("🔄 Batch-Verarbeitung gestartet im Hintergrund...")
                    self.cache.start_background_task(
                        task_id,
                ...
                st.info("🔄 Batch-Verarbeitung läuft im Hintergrund...")
                elapsed = datetime.now() - task_result['started']
                st.write(f"⏱️ Laufzeit: {elapsed.seconds}s")
                ...
                st.success("✅ Batch-Verarbeitung abgeschlossen!")
                all_faces = task_result['result']
                
                return self._select_face_from_batch(all_faces, uploaded_files)
            
            elif task_result['status'] == 'failed':
                st.error(f"❌ Batch-Verarbeitung fehlgeschlagen: {task_result['error']}")
        
        return None
    
    def _process_batch_images(self, uploaded_files) -> List[Dict]:
        """Verarbeite mehrere Bilder im Hintergrund."""
        all_faces = []
        
        for i, file in enumerate(uploaded_files):
            try:
                faces = self.search_engine.extract_all_faces(file, use_cnn=False)  # HOG für Speed
                for face in faces:
                    face['source_file'] = file.name
                    face['file_index'] = i
                    all_faces.append(face)
            except Exception as e:
                # Ignoriere Fehler bei einzelnen Bildern
                continue
        
        return all_faces
    
    def _select_face_from_batch(self, all_faces: List[Dict], uploaded_files) -> Optional[Dict]:
        """Wähle Gesicht aus Batch-Ergebnissen."""
        if not all_faces:
            st.warning("❌ Keine Gesichter in den Batch-Bildern gefunden")
            return None
        
        st.write(f"👥 **{len(all_faces)} Gesichter** in {len(uploaded_files)} Bildern gefunden")
        
        # Gruppiere nach Bild
        faces_by_file = {}
        for face in all_faces:
            file_name = face['source_file']
            if file_name not in faces_by_file:
                faces_by_file[file_name] = []
            faces_by_file[file_name].append(face)
        
        # Zeige Auswahl
        selected_file = st.selectbox(
            "Bild auswählen:",
            list(faces_by_file.keys())
        )
        
        if selected_file:
            file_faces = faces_by_file[selected_file]
            
            if len(file_faces) == 1:
                selected_face = file_faces[0]
                st.info(f"✅ Gesicht aus {selected_file} ausgewählt")
            else:
                face_options = [f"Gesicht {i+1}" for i in range(len(file_faces))]
                selected_idx = st.selectbox("Gesicht auswählen:", range(len(file_faces)), 
                                          format_func=lambda x: face_options[x])
                selected_face = file_faces[selected_idx]
            
            return {
                'encoding': selected_face['encoding'],
                'selected_face': selected_face,
                'all_faces': [selected_face],
    ...
    def _process_uploaded_image(self, uploaded_file) -> Optional[Dict]:
        """Verarbeite einzelnes hochgeladenes Bild."""
        ...
        with st.spinner("Analysiere Gesichter im Bild..."):
            ...
            if not faces:
                ...
                with st.expander("💡 Tipps für bessere Gesichtserkennung:", expanded=True):
                    ...
                    st.warning("🔄 **Problem:** Bild war sehr klein - versuche ein größeres Bild!")
    ...
    def render_upload_section(self) -> Optional[Dict]:
        """Rendere Upload-Bereich - nutzt erweiterte Upload-Funktionen."""
        return self.render_advanced_upload_section()
...
def render_search_page():
    ...
    Mit Performance Dashboard, Smart Caching und Background Processing.
    """
    st.title("🔍 Intelligente Gesichtssuche v2.0")
    st.markdown("---")
    
    # Info über das neue System
    st.info("🚀 **Ultra-Präzise Suche mit KI-Performance:** Smart Caching, Background Processing, Batch-Upload!")
    
    # Erstelle UI-Instanz
    ui = FaceSearchUI()
    
    # Performance Dashboard (optional)
    if st.checkbox("⚡ Performance Dashboard anzeigen"):
        ui.render_performance_dashboard()
        st.markdown("---")
    
    # Erweiterte Upload-Bereich
    face_data = ui.render_advanced_upload_section()
            ...
            if results:
                ...
                with st.expander("💡 Versuche diese Tipps:", expanded=True):
                    ...
                    **Mögliche Gründe und Lösungen:**
                    ...
                    6. **🧠 Cache leeren:** Manchmal hilft Cache aufräumen
                    ...
                    **📊 Status prüfen:**
                    ...
                    - Nutze Performance Dashboard für Cache-Statistiken
                    """)
    ...
    # Footer mit Performance-Info
    if ui.search_engine.stats['total_searches'] > 0:
        st.markdown("---")
        st.caption(f"⚡ Performance: {ui.search_engine.stats['total_searches']} Suchen, "
                  f"Ø {ui.search_engine.stats['avg_search_time']:.2f}s, "
                  f"Cache Hit-Rate: {ui.cache.get_cache_stats()['hit_rate']:.1f}%")
