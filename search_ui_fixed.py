...
class SmartFaceSearch:
    ...
    def extract_all_faces(self, image_data, use_cnn=True) -> List[Dict]:
        ...
        try:
            ...
            # Verwende CNN für höchste Präzision bei Upload-Bildern
            model = "cnn" if use_cnn else "hog"
            
            # Finde Gesichter
            face_locations = self.face_recognition.face_locations(img_array, model=model)
            
            if not face_locations:
                return []
            
            # Extrahiere Encodings mit "large" Modell für beste Qualität
    ...
    def render_upload_section(self) -> Optional[Dict]:
        ...
        if uploaded_file is not None:
            ...
            with st.spinner("Analysiere Gesichter im Bild..."):
                ...
                if not faces:
                    st.error("❌ Keine klaren Gesichter im Bild erkannt! Versuche ein anderes Bild.")
                    return None
                elif len(faces) == 1:
